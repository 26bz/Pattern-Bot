  ...
  info: (message, meta = {}) => {
    ...
    if (typeof message === 'string' && meta && Object.keys(meta).length > 0) {
      if (message.includes('Bot successfully logged in')) {
        logger.success(
          `🤖 ${meta.botTag} online | 🏠 ${meta.guildCount} servers | 👥 ${meta.userCount} users | 🎯 ${meta.patternsLoaded} patterns`,
        )
      } else {
        logger.info(message)
      }
...
class PerformanceLogger {
  ...
  static logMemoryUsage() {
    ...
    logger.info(
      `💾 Memory: ${Math.round(usage.heapUsed / 1024 / 1024)}MB used / ${Math.round(
...
class PatternStats {
  ...
  trackPatternMatch(message, pattern, confidence, processingTime = null, response = '') {
    ...
    this.patternStats[pattern].avgConfidence =
      this.patternStats[pattern].totalConfidence / this.patternStats[pattern].count
    ...
    logger.success(
      `🎯 "${messagePreview}" → "${responsePreview}" | 👤 ${username} in #${channelShort} | 📊 ${confidence.toFixed(
        2,
      )} | ⏱️ ${processingTime || 0}ms`,
    )
