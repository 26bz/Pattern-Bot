...
client.on('messageCreate', message => {
  ...
  try {
    ...
    const isQuestion =
      content.includes('?') ||
      /^(what|who|when|where|why|how|can|could|would|is|are|am|do|does|did|will|should).+/.test(content)

    const confidenceThreshold = isBotMention
      ? config.confidenceThreshold.mention
      : isQuestion
      ? config.confidenceThreshold.question
      : config.confidenceThreshold.default

    if (config.debug) {
      logger.debug(
        `Message: "${content}" | Clean: "${cleanContent}" | Mention: ${isBotMention} | Question: ${isQuestion} | Threshold: ${confidenceThreshold}`,
      )
    }
    ...
    for (const [pattern, response] of questionsCache.entries()) {
      try {
        ...
        if (match) {
          ...
          if (config.debug) {
            logger.debug(
              `Pattern: "${pattern}" | Match: "${match[0]}" | Confidence: ${confidence.toFixed(
                2,
              )} | Threshold: ${confidenceThreshold} | Pass: ${confidence >= confidenceThreshold}`,
            )
          }
